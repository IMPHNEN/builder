...
> Forward-looking feature plan. Bug-fix hardening (Phases 0–5) is complete; see git history and the test suite. This document is **new features only**, ordered by dependency and value. Each feature lists the files it touches (paths verified against the current codebase) and its status.
...
### A1. Multi-provider support `[x]`

- **What:** register OpenAI, Google, Mistral, etc. in `createProviderRegistry`; resolve keys per-provider.
- **Touches:** `llm/registry.ts`, `llm/provider-config.ts`, `llm/models.ts`, `stores/provider.ts`, and the model settings UI.
- **Implemented:** Anthropic, Claude-compatible, OpenAI, OpenAI-compatible, Google, and Mistral provider adapters with client-supplied keys and endpoints.
- **Effort:** S · **Value:** High

### A2. Model picker in UI `[x]`

- **What:** dropdown to choose `provider:model`; passed to `streamText({ modelString })`.
- **Touches:** `components/header/`, `lib/stores/provider.ts`, `routes/api.chat.ts`, and `stream-text.ts`.
- **Implemented:** global persisted model selection with static catalogs and live discovery for OpenAI-shaped endpoints.
- **Effort:** M · **Value:** High

### A3. Per-model token/segment limits `[x]`

- **What:** `MAX_TOKENS` / `MAX_RESPONSE_SEGMENTS` are global constants (`llm/constants.ts`); make them per-model.
- **Touches:** `llm/constants.ts` (`MODEL_LIMITS` keyed by provider/model id); `api.chat.ts` reads via the active model.
- **Implemented:** `MODEL_LIMITS` drives both output tokens and continuation segments for the active model. **Effort:** S · **Value:** Medium
...
- **Touches:** `persistence/db.ts`, `persistence/messages.ts`, and `components/sidebar/Menu.client.tsx` (import button + file picker).
- **Note:** validate with `zod` (already a devDependency) against the `ChatExport` shape.
- **Implemented:** versioned imports are validated, legacy message content is normalized, and fresh ids/urlIds prevent collisions. **Effort:** S · **Value:** Medium

### B2. Server-side persistence (Cloudflare D1) `[~]`

- **What (revised):** keep IndexedDB as the local-first persistence layer for chats and provider settings; add optional D1 sync for authenticated accounts and shared/remote chats. **D1 is SQLite-at-the-edge** — the browser remains the local cache and offline source of truth.
...
- **Blocked on:** auth (C2) — remote persistence and sync are per-user.
- **Effort:** L · **Value:** High

### B3. Project export to ZIP `[x]`

- **What:** serialize the WebContainer FS (`lib/stores/files.ts` `FileMap`) into a downloadable zip.
- **Touches:** `utils/zip.ts`, `lib/stores/files.ts`, and a workbench header button (`Workbench.client.tsx`).
- **Implemented:** browser-safe `fflate` serialization with a workbench download action and retained binary watcher bytes when available.
- **Effort:** M · **Value:** High

### B4. GitHub import / export `[x]`

- **What:** clone a repo into the WebContainer; push the project filesystem to a repo.
- **Touches:** `lib/github/client.ts`, `lib/stores/provider.ts`, `lib/stores/workbench.ts`, `components/workbench/`.
- **Implemented:** user-supplied OAuth/PAT tokens are persisted in browser settings and passed to Git through temporary `http.extraHeader` environment configuration, never embedded in remote URLs.
- **Effort:** L · **Value:** High
...
```
Now (no decisions needed):     E2 → E1 → D2 → E4
Completed:                     A1 → A2 → A3 → B1 → B3 → B4
Agentic core (decide wire format first): F1 → F2 → F3 → F5
                                         F1 → F6 (with D1)
                                         F4 (after F1 + F2)
Blocked on auth (C2):        C2 → B2 → C1
Blocked on UX/e2e:           D1, D3, E3
```

**Recommended next sprint:** E2, E1, D2, E4 — all independent hardening work; B2 remains gated on authentication and remote-sync decisions.
...
_Generated after completion of the P0–P5 hardening roadmap. Update status markers as features land._
